	...
	const bool debug)
{
  ...
  torch::Tensor out_alpha_t = torch::full({1, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_alpha_t.contiguous().data<float>(),
		out_normal.contiguous().data<float>(),
		out_distortion.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_depth, out_middepth, out_alpha, out_alpha_t, out_normal, out_distortion, radii, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_ddepths_plane = torch::zeros({P, 2}, means3D.options());
