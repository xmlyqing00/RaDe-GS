...
renderCUDA(
	...
	float* __restrict__ out_alpha_t,
	...
	bool prefiltered)
{
	...
	float2 point_image = {focal_x * p_view.x / (p_view.z + 0.0000001f) + (float)W/2., focal_y * p_view.y / (p_view.z + 0.0000001f) + (float)H/2.};
	...
	float* __restrict__ out_sdf)
{
	...
	while (true)
	{
		...
		float3 ray_point = { (pixf.x - (float)W/2.) / focal_x, (pixf.y - (float)H/2.) / focal_y, 1.0 };
