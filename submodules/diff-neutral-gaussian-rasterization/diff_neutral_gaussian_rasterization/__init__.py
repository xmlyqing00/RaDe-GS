...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        if raster_settings.debug:
            ...
            try:
                num_rendered, color, depth, alpha, alpha_t, normal, radii, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(*args)
            except Exception as ex:
                torch.save(cpu_args, "snapshot_fw.dump")
                print("\nAn error occured in forward. Please forward snapshot_fw.dump for debugging.")
                raise ex
        else:
            num_rendered, color, depth, middepth, alpha, alpha_t, normal, distortion, radii, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(*args)
        ...
        return color, radii, depth, middepth, alpha, alpha_t, normal, distortion

    @staticmethod
    def backward(ctx, grad_color, grad_radii, grad_depth, grad_middepth, grad_alpha, grad_alpha_t, grad_normal, grad_distortion):
        ...
        # assert not torch.isnan(grad_cov3Ds_precomp).any()
        grads = (
            grad_means3D,
            grad_means2D,
            grad_sh,
